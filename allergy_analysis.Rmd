...
This code creates two histograms showing the age distribution of patients at the start (in blue) and end (in red) of the observation period, with counts displayed in thousands. It also calculates seperately the minimum and maximum ages for both timepoints.
...
```{r}
# Calculate rhinitis and allergen presence for START and END
rhinitis_allergen_analysis_start <- allergy_data %>%
  mutate(
    has_rhinitis = !is.na(ALLERGIC_RHINITIS_START),
    has_allergen = rowSums(!is.na(select(., ends_with("_ALG_START")))) > 0
  ) %>%
...
# Create visualizations for START and END
ggplot(rhinitis_allergen_analysis_start, 
       aes(x = factor(has_rhinitis), y = allergen_presence)) +
  geom_bar(stat = "identity", fill = "steelblue") +
  theme_minimal() +
  labs(title = "Allergen Presence by Allergic Rhinitis Status (START)",
...
# Detailed allergen breakdown for rhinitis patients (START)
rhinitis_allergen_analysis_start <- allergy_data %>%
  filter(!is.na(ALLERGIC_RHINITIS_START)) %>%
  summarise(
    peanut = sum(!is.na(PEANUT_ALG_START))/n(),
    egg = sum(!is.na(EGG_ALG_START))/n(),
    milk = sum(!is.na(MILK_ALG_START))/n(),
...
ggplot(rhinitis_allergen_analysis_start, 
  ...
  coord_flip() +
  theme_minimal() +
  labs(title = "Allergen Distribution in Allergic Rhinitis Patients (START)",
       x = "Allergen Type",
       y = "Prevalence") +
  scale_y_continuous(labels = scales::percent)
